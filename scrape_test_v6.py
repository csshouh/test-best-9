#!/usr/bin/env python3
"""GitHub Actions Playwright 抓取脚本 V6（混合方案：networkidle + 智能 API 等待）

改进：先用 networkidle 快速等待（短 timeout 5s），再用智能 API 等待确保数据已返回。
双保险机制：networkidle 处理快速加载场景，API 等待处理慢响应场景。
"""
import argparse
import datetime
import json
...
def wait_for_api(all_api_data, url_pattern, timeout=10):
    """轮询等待特定 API 响应"""
    start = time.time()
...
def wait_hybrid(page, all_api_data, url_pattern, networkidle_timeout=5, api_timeout=10):
    """混合等待：networkidle 快速等待 + 智能 API 等待确保数据返回

    1. 先用 networkidle 短 timeout 快速等待（处理快速加载场景）
    2. 再用智能 API 等待确保目标数据已返回（处理慢响应场景）
    """
    # 第一步：networkidle 快速等待
    try:
        page.wait_for_load_state("networkidle", timeout=networkidle_timeout * 1000)
    except Exception:
        pass  # networkidle 超时，继续用 API 等待

    # 第二步：智能 API 等待
    return wait_for_api(all_api_data, url_pattern, timeout=api_timeout)


def wait_for_new_response(all_api_data, url_pattern, before_count, timeout=10):
    """等待特定 API 出现新响应（通过响应计数）"""
    start = time.time()
    while time.time() - start < timeout:
        current_count = sum(len(all_api_data[u]) for u in all_api_data if url_pattern in u)
...
def scrape_one(page, goods_id, item_name=None):
    ...
    try:
        # 1. 访问详情页 + 混合等待 info/good API
        print(f"  [1] 访问详情页（混合等待 info/good API）...", flush=True)
        try:
            page.goto(detail_url, wait_until="networkidle", timeout=30000)
        except Exception:
            page.goto(detail_url, wait_until="domcontentloaded", timeout=30000)
        # 混合等待：networkidle + API 等待
        info_resp = wait_for_api(all_api_data, API_INFO_GOOD, timeout=10)
        ...
        # 3. 点击 K 线图 + 混合等待 chartAll API
        print(f"  [3] 点击 K 线图（混合等待 chartAll API）...", flush=True)
        page.evaluate("""() => {
        ...
        chart_resp = wait_hybrid(page, all_api_data, API_CHART_ALL, networkidle_timeout=5, api_timeout=10)
        if chart_resp is None:
            print(f"      chartAll API 超时，回退等待", flush=True)
            page.wait_for_timeout(2000)

        # 4. 切换平台到悠悠有品 + 混合等待 chartAll API 新响应
        print(f"  [4] 切换平台到悠悠有品（混合等待 chartAll API 新响应）...", flush=True)
        ...
        if select_info:
            ...
            # 混合等待：networkidle + 新响应计数
            try:
                page.wait_for_load_state("networkidle", timeout=5000)
            except Exception:
                pass
            if not wait_for_new_response(all_api_data, API_CHART_ALL, before_chart_count, timeout=10):
                page.wait_for_timeout(2000)
            print(f"      ✓ 切换完成", flush=True)

        # 5. 切换日线 + 混合等待 chartAll API 新响应
        print(f"  [5] 切换日线（混合等待 chartAll API 新响应）...", flush=True)
        before_chart_count = sum(len(all_api_data[u]) for u in all_api_data if API_CHART_ALL in u)
        page.evaluate("""() => {
        ...
        # 6. 翻页（每次混合等待 chartAll API 新响应）
        print(f"  [6] 翻页（混合等待 chartAll API 新响应）...", flush=True)
        ...
        if canvas_info:
            ...
            for i in range(CHART_SCROLL_TIMES):
                ...
                # 混合等待：networkidle 短 timeout + 新响应计数
                try:
        ...
        # 7. 切换 1 小时 + 混合等待 chartAll API 新响应
        print(f"  [7] 切换 1 小时（混合等待 chartAll API 新响应）...", flush=True)
        before_chart_count = sum(len(all_api_data[u]) for u in all_api_data if API_CHART_ALL in u)
        page.evaluate("""() => {
        ...
        # 8. 筹码分布 + 混合等待 chipData API 新响应
        print(f"  [8] 点击筹码分布图（混合等待 chipData API 新响应）...", flush=True)
        ...
        if not wait_for_new_response(all_api_data, API_CHIP_DATA, before_chip_count, timeout=10):
            ...
            try:
                ...
                pass
            if not wait_for_new_response(all_api_data, API_CHIP_DATA, before_chip_count, timeout=8):
...
def main():
    parser = argparse.ArgumentParser(description="CSQAQ Playwright 抓取 V6")
    parser.add_argument("--items-json", default="", help="批量 JSON 数组")
    args = parser.parse_args()

    print("=" * 60, flush=True)
    print("  CSQAQ Playwright 抓取 V6（混合方案：networkidle + 智能 API 等待）", flush=True)
    print("=" * 60, flush=True)
